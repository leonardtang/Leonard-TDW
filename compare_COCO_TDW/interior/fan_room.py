...
class ProcGenInteriorDesign(Controller):
    ...
    def run(self):
        ...
        self.communicate(TDWUtils.create_avatar(position={"x": 0, "y": 1.7, "z": 0},
                                                look_at=TDWUtils.array_to_vector3([4, 0, 0]),
                                                avatar_id="avatar"))
        ...
        scene_data = self.communicate({"$type": "look_at_position",
                                       "avatar_id": "avatar",
                                       "position": TDWUtils.array_to_vector3([4, 0, 0])})

        images = Images(scene_data[0])
        TDWUtils.save_images(images, "fan_room", output_directory="replicated_images")
